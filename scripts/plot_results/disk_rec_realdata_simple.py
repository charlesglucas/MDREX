...
def main(cfg):
    ...
    title = display_titles.get(result_file, result_file.replace('_', ' '))
    data = np.load(ROOT / f"results/realdata/{result_file}.npz", allow_pickle=True)
    y = data["y"]
    x_disk_store = data["x"]
    ...
    for k in range(3):
        for j in range(3):
            ...
            sm = nsmooth+k; 
            sp = nsparse+j
            plt.subplot(3,3,h); plt.imshow(np.squeeze(np.mean(x_disk_store[k+2,j+3], axis=0))); plt.title(rf"$(10^{{{sm}}},\, 10^{{{sp}}})$"); plt.colorbar()
    ...
    def render_rgb(data1, data2, annotation, suffix, use_scaled_ticks=False):
        ...
        elif data_file == "HD_106906-2016-03-28-h2_h3":
            height, width = rgb.shape[:2]
            ax.set_xlim(width / 4, 3 * width / 4)
            ax.set_ylim(height / 4, 3 * height / 4)
        ...
        fig.savefig(f"figures/{result_file}_{suffix}_musmooth{sm}_musparse{sp}_RGB.pdf", dpi=300,
    ...
    for row, (result, annotation) in enumerate(combined_plots):
        ...
        if result_file == "RY_lup" or result_file == "HD_106906-2016-03-28-h2_h3" or result_file == "HD_106906-2016-03-28-k1_k2":
    ...
    combined_fig.savefig(
        f"figures/{result_file}_musmooth{sm}_musparse{sp}_RGB.pdf",
        dpi=300, bbox_inches="tight", pad_inches=0.1,
    )
    plt.show()


    for (k,j) in [(3,3), (4,3), (3,4), (4,4), (5,5)]:
        sm = nsmooth + k
        sp = nsparse + j
        x_result = x_disk_store[k, j]
        ax_result = Ax_disk_store[k, j]
        hdu = fits.PrimaryHDU(data=x_result)
        hdu.writeto(f"results/realdata/fits/{result_file}_x_sm{sm}_sp{sp}.fits", overwrite=True)
