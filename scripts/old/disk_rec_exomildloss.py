...
def main(cfg):
    ...
    optimizer = optim.Adam([x_disc], lr=lr)
    ...
    with torch.autograd.detect_anomaly(check_nan=True):
        for i in range(200):
            ...
            ## forward model
            im = x_disc.view(1, 1, H, W).expand(-1, T, -1, -1)
            # (b, T, H, W)
            ...
            print(f"Learning rate: {current_lr:.11f}")
            print(x_disc.grad.abs().mean().item())
    ...
    plt.subplot(2,2,4); plt.imshow(np.squeeze(diff.detach().cpu().numpy()[0,0,0,:,:])); plt.title("$\phi( y-Ax )$"); plt.colorbar()
    plt.show()
