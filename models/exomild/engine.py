...
class EngineSpatial(nn.Module):
    ...
    def _params_gaussian(self, x, mean=None):
        ...
        with context:
            ...
            # Compute traces without materialising S_hat**2 or S_hat @ S_hat:
            # tr(S^2) = ||S||_F^2 for symmetric S (uses hadamard product, not matmul)
            # tr(diag(S)^2) = sum of squared diagonal elements
            diag_S = torch.diagonal(S_hat, dim1=-2, dim2=-1)  # (bs, bsp, G, fsp)
            tr_S2 = (diag_S ** 2).sum(dim=-1)
            tr2_S = diag_S.sum(dim=-1) ** 2
            tr_SS = (S_hat * S_hat).sum(dim=(-2, -1))  # Frobenius^2 = tr(S S^T) = tr(S^2) for symmetric S
    ...
    def fit_params(self, xp, idx):
        ...
        # prenorm is identity (zeros/ones) — kept for API compatibility with get_log_likelihood
        prenorm_mean = torch.zeros(1, bsp, 1, self.S, 1, device=xp.device, dtype=xp.dtype)
        prenorm_std = torch.ones(1, bsp, 1, self.S, 1, device=xp.device, dtype=xp.dtype)
        ...
        bg_params["mean"] = bg_params["mean"].squeeze(0)
        bg_params["C_inv"] = bg_params["C_inv"].squeeze(0)
        # Do NOT store patches here: they cost ~125 MB per term and are not
        # needed in run_bfgs (get_log_likelihood recomputes features with grad).
